...
public class GenerateAndSaveService {
     ...
     *
     * <p>If the file exists, the user is prompted to overwrite it. If the user declines, the method
     * attempts to generate a unique file name by appending a numeric suffix (e.g., "(1).png").
     *
     * <p>A safety limit of 1000 attempts is enforced to prevent infinite loops and excessive system
     * resource consumption in directories with high file counts.
     *
     * @param file the initial File to check for conflicts; must not be null
     * @return a File ready for writing (original, user-approved, or auto-renamed), or null if:
     *     <ul>
     *       <li>the input file is null
     *       <li>the maximum number of naming attempts (1000) is reached
     *     </ul>
     */
    private File resolveFileNameConflict(File file) {
        ...
        if (file.exists()) {
            ...
            do {
                ...
                counter++;
            } while (candidate.exists() && counter <= maxAttempts);
            if (candidate.exists()) {
                MyPopup.INSTANCE.showDialog(
                        "Erreur de nommage",
                        "Impossible de générer un nom unique après " + maxAttempts + " essais.",
                        "Erreur");
                return null;
            }
            return candidate;
